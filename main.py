...
def drive(target_distance, speed_factor=1.0):
    """
    Drive forward a certain distance in millimeters using PID.\n#
    drive(614) would drive forward 614mm, or 2 feet.\n
    An *optional* parameter speed_factor can be applied. The factor is a floating point decimal that multiplies the output of PID, effectively modifying the speed.\n
    drive(614, speed_factor=0.5) would drive forward 614mm at half the speed of drive(614). The default value for speed_factor is 1.0.\n
    """
    # Reset Positions
    ...
    while True:
        ...
        left_output = LeftPID.compute(target_distance, left_pos) / 1.4 * speed_factor
        right_output = RightPID.compute(target_distance, right_pos) / 1.4 * speed_factor

        # Apply Output & Spin
        dt_left.spin(FORWARD, left_output, PERCENT)
        dt_right.spin(FORWARD, right_output, PERCENT)
        ...
        if (abs(left_pos - target_distance) < pid_sensitivity * 3 and abs(right_pos - target_distance) < pid_sensitivity * 3) or (0 < abs(drivetrain.velocity()) < 5):
            dt_left.stop()
            dt_right.stop()
            drivetrain.stop()
            break
...
def turn(target_angle, direction, turn_pid_factor=1.0):
    """
    Turn a certain angle in degrees using PID.\n#
    The second argument, direction, can be either LEFT or RIGHT.\n
    turn(90, RIGHT) would turn the robot 90 degrees to the right.\n
    An *optional* parameter turn_pid_factor can be applied. The factor is a floating point decimal that multiplies the output of PID, effectively modifying the speed.\n
    turn(90, RIGHT, turn_pid_factor=0.5) would turn the robot 90 degrees to the right at half the speed of turn(90, RIGHT). The default value for turn_pid_factor is 1.0.\n
    """
    # Reset Angle
    ...
    while True:
        # Log New Angle
        measured_angle = round(inertial.heading() if direction == RIGHT else abs(360 - inertial.heading()))

        # Compute New Output from PID
        output = round(min(200, max(-200, RotationalPID.compute(target_angle, measured_angle) * turn_pid_factor)))
